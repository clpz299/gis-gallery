...
## TIF 预览（页面右侧）
- 页面采用左右布局：左侧提交下载任务，右侧使用 OpenLayers 预览 GeoTIFF
- 预览使用方式：
  1. 选择输出格式为 GeoTIFF（`.tif`）
  2. 建议勾选“合并输出”，这样会在任务目录根部生成单个 `区域名_z{zoom}.tif`，更适合直接预览
  3. 下载任务提交后，点击“刷新输出文件”，选择 `.tif` 文件并点击“在右侧预览该 TIF”
- 不合并输出（XYZ 目录）场景下，输出通常位于 `tiles/z/x/y.png`。此时“输出文件列表”会列出子目录中的文件路径，主要用于查看下载结果是否落盘

## 任务输出文件接口（预览/下载）
- 列出任务输出文件：
  - `GET /api/gridtile/tasks/{taskId}/files`
  - 返回：`name/size/url/type`
- 读取任务输出文件（支持多级目录与 Range，用于大文件按需读取）：
  - `GET /api/gridtile/tasks/{taskId}/files/**`
  - 说明：当文件位于子目录（例如 `tiles/10/845/390.png`）时，`**` 用于匹配完整相对路径
...
graph TD
    ...
    Frontend -->|2. 查询行政区划| Controller[后端: RegionDwonloadTifController]
    Controller -->|3. 返回区域树| Frontend
    Frontend -->|4. 提交下载任务| Controller
    Controller -->|5. 派发任务| Service[服务: TileDownloadService]
    
    subgraph "核心处理引擎"
        Service -->|6. 计算 BBox| GeoJsonUtils[工具: GeoJsonBboxUtils]
